...
function getDayName(date) {
  ...
  const dayIdx = newDate.getUTCDay();
  return daysOfTheWeek[dayIdx];
}
...
function getWeekNumberByDate(date) {
  ...
  const diff = date - firstDayOfTheYear;
  const firstDayOfYear = firstDayOfTheYear.getDay();
  const amountOfMsInWeek = 1000 * 60 * 60 * 24 * 7;
  const result =
    firstDayOfYear === day
      ? Math.ceil(diff / amountOfMsInWeek) + 1
      : Math.ceil(diff / amountOfMsInWeek);
  return result;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startDay = Number(start.split('-')[0]);
  const startMonth = Number(start.split('-')[1]);
  const year = Number(start.split('-')[2]);

  const endDay = Number(end.split('-')[0]);
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (startDate <= endDate) {
        const workDay = `${String(startDate.getDate()).padStart(2, '0')}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${startDate.getFullYear()}`;
